...
var ItemTree = class ItemTree extends LibraryTree {
  static async init(domEl, opts = {}) {
    ...
    await new Promise((resolve) => {
      ReactDOM.createRoot(domEl).render( /*#__PURE__*/React.createElement(ItemTree, _extends({ ref: (c) => {
          ref = c;
          resolve();
        } }, opts)));
    });
  ...
  async setItemsPaneMessage(message, lock = false) {
    ...
    if (typeof message == 'string') {
      ...
      if (message == Zotero.getString('pane.items.loading')) {} else
      {
  ...
  refresh = Zotero.serial(async function (skipExpandMatchParents) {
    ...
    try {
      ...
      for (let i = 0; i < this._rows.length; i++) {
        ...
        if (row.level == 1 && !row.ref.parentID) {
  ...
  render() {
    ...
    const itemsPaneMessage = /*#__PURE__*/React.createElement("div", {
    ...
    let virtualizedTable = /*#__PURE__*/React.createElement("div", { style: { display: showMessage ? 'none' : 'flex' },
      className: "virtualized-table", key: "virtualized-table-stub" });
    if (this.collectionTreeRow) {
  ...
  canDropCheck = (row, orient, dataTransfer) => {
    ...
    if (dataType == "text/x-moz-url" || dataType == 'application/x-moz-file') {
  ...
  _renderPrimaryCell(index, data, column) {
    ...
    try {
      ...
      if (item instanceof Zotero.Search) {
      ...
      if (this.collectionTreeRow.isLibrary() && !Zotero.Groups.getAll().length) {
        ...
        for (let span of div.getElementsByTagName('span')) {
          if (span.classList.contains('text-link')) {
            ...
            if (span.hasAttribute('data-action')) {
